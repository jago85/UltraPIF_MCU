...
//    config_i2c_master.buffer_timeout = 10000;
    ...
    //config_i2c_master.pinmux_pad0 = SERCOM3_PAD0_DEFAULT;
    //config_i2c_master.pinmux_pad1 = SERCOM3_PAD1_DEFAULT;

    // Initialize and enable device with config.
//    i2c_master_init(&i2c_master_instance, I2C_SERCOM, &config_i2c_master);
...
//    memcpy((write_buffer + 1), data, bytes);
//
//    stat = HAL_I2C_Master_Transmit(hi2c, 0xC0, write_buffer, bytes + 1, 1000);
//
//    return (stat == HAL_OK) ? 0 : 1;
//}

extern void I2cTransmit(uint8_t devAddr, uint8_t *data, uint8_t length);
extern void I2cReceive(uint8_t devAddr, uint8_t *data, uint8_t length);
...
uint8_t si5351_write_bulk(uint8_t addr, uint8_t bytes, uint8_t *data)
{
    ...
    I2cTransmit(0xC0, write_buffer, bytes + 1);
...
//    buf[0] = addr;
//    buf[1] = data;
//    stat = HAL_I2C_Master_Transmit(hi2c, 0xC0, buf, 2, 1000);
//
//    return (stat == HAL_OK) ? 0 : 1;
//}
...
//    uint16_t timeout = 0;
//
//    // Init I2C packet to write register address
...
//
//    buf[0] = addr;
//    stat = HAL_I2C_Master_Transmit(hi2c, 0xC0, buf, 1, 1000);
//    if (stat != HAL_OK)
//        return stat;
//    stat = HAL_I2C_Master_Receive(hi2c, 0xC0, &buf[1], 1, 1000);
//    *data = buf[1];
//    return (stat == HAL_OK) ? 0 : 1;
//}
...
uint8_t si5351_read(uint8_t addr, uint8_t *data)
{
    ...
    buf[0] = addr;
    I2cTransmit(0xC0, buf, 1);
    I2cReceive(0xC0, &buf[1], 1);
    *data = buf[1];
